...
Hotkey deleteSelectedFile{VK_DELETE};      // VK_DELETE
...
void Editor::ProcessWindow()
{
                    ...
                    while (clipper.Step())
                    {
                        for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; ++i)
                        {
                            ...
                            if (pEntry->bRename)
                            {
                                ...
                                if (ImGui::IsKeyPressed(VK_RETURN) || (ImGui::IsMouseClicked(ImGuiMouseButton_Left) && !ImGui::IsItemHovered()))
                                {
    ...
    if (!blockHotkeys)
    {
        ...
        else if (deleteSelectedFile.Pressed())
        {
            DeleteSelected();
        }
    }
